...
#                                       growth)
        ...
        def write_evs(self,EvES,EvSD,EvR,res_path):
                f_exsu = open((res_path+'/Ev_ES.dat'),'w')
                f_scdi = open((res_path+'/Ev_SD.dat'),'w')
                f_prices = open((res_path+'/Ev_RR.dat'),'w')
                for u in self.Scn.U:
                        for k in self.K:
                                aux_p = '[%i,%i];'%(u,k)
                                aux_s = '[%i,%i];'%(u,k)
                ...
                f_exsu.close()
                f_prices.close()
        def write_data(self,data,name, res_path):
...
def Example(identical_scen, congestion):
    ...
    for u in U2:
        if identical_scen:
            ...
            gr[u] = random.uniform(1,1.2)
        print('growth at scen ', u, ': ', gr[u])
    
    # Probablity
    prob = {}
    for u in U2:
        prob[u] = random.uniform(0,1) 
    prob_sum = sum(prob[u] for u in U2)
    for u in U2:
        if identical_scen:
            prob[u] = 1/len(U)
        else:
            prob[u] = prob[u]/prob_sum
        print("Probability at", u , '  ',prob[u])
    growth={}
    growth[1] = sum(prob[u] * gr[u] for u in U2)
    print("Expected growth", growth[1])
    pr={1:1}
    Scen = {}
