    ...
    def step_1(self, ADMM, ipoptObjs):
        ...
        for u in self.Scn.U:
            ...
            def obj_rule_tap(model):
                ...
                exp1 = ( sum( model.q[r,s,k]*(pyolog(model.q[r,s,k]) - 1.0 -self.C.b0[k]) for k in self.K for s in self.S for r in self.R))
#                 exp2 = -sum(ADMM.rho[u,k] * sum(self.C.e[r,s]*model.q[r,s,k] for r in self.R for s in self.S) for k self.K)
                ...
                return self.C.b1/self.C.b3*exp0 + 1.0/self.C.b3*exp1 + exp2 + ADMM.r_1/2.0 * exp3
        ...
        for u in self.Scn.U:
            ...
            def obj_rule_tap(model):
                exp0 = sum(self.C.tff[r,s]*(model.v[r,s]+(self.C.b[r,s]/(self.C.alpha[r,s]+1.0))*(model.v[r,s]**(self.C.alpha[r,s]+1))/(self.C.cap[r,s]**(self.C.alpha[r,s]))) for (r,s) in self.C.A)
                exp1 = ( sum( model.q[r,s,k]*( pyolog(model.q[r,s,k]+0.00000001) - 1.0 -self.C.b0[k]) for k in self.K for s in self.S for r in self.R))
#                 exp2 = -sum(ADMM.rho[u,k] * sum(self.C.e[r,s]*model.q[r,s,k] for r in self.R for s in self.S) for k self.K)
                exp3 =  sum(((-sum(self.C.e[r,s]*(2*model.q[r, s, k] - ADMM.q[u,r,s,k]) for r in self.R for s in self.S ) + ADMM.g[u, k])/2)** 2 for k in self.K )
                exp2 = sum(ADMM.rho[u,k] * sum(self.C.e[r,s]*model.q[r,s,k] for r in self.R for s in self.S) for k in self.K) 
                return self.C.b1/self.C.b3*exp0 + 1.0/self.C.b3*exp1 + exp2 + ADMM.r_1/2.0 * exp3
...
#                   growth)
    ...
    def exsu(self, ADMM):
        ...
        for u in self.Scn.U:
            for k in self.K:
                ES[u,k] = (ADMM.g[u,k] - sum( self.C.e[r,s]*ADMM.q[u,r,s,k] for r in self.R for s in self.S))#/(ADMM.g[u,k]+1)
        return ES
    ...
    def scen_diff(self, ADMM):
        ...
        for u in self.Scn.U:
            for k in Ntw.K:
                SD[u, k] = (ADMM.c[u,k] - ADMM.znu[k])#/(ADMM.c[u,k]+1)
        return SD

    # all the output can be saved in this function.
    def write_evs(self,EvES,EvSD,EvR,c,g,q,res_path):
        ...
        for u in self.Scn.U:
            for r in self.R:
                for s in self.S:
                    for k in self.K:
                        aux_q = 'Scenario%i,Orig%i,Dest%i,Node%i, '%(u,r,s,k)
                        for nu in EvES:
...
class Consumers(Network):
    ...
    def traffic_problem(self, u, optsolver):
        ...
        if (warm_start == True):
            ...
            optsolver.options['halt_on_ampl_error'] = 'no' 
            optsolver.options['linear_solver'] = 'ma57' 
            optsolver.options['print_level'] = '5' # 5 is default level, the larger the number the more detail output 
...
#     Ntw = Example(identical_scen, congestion)
    ...
    EM_perc = {} # max percentage excess supply for all the locations
    SM_perc = {} # max percentage  scenario difference for all the locations
    ...
    for iter in range(Maxit):
        ...
        EM[iter] = maxee
        SM[iter] = maxsd
        # zg: calculate the percentage difference
        maxee_perc = max( abs(ES[u, k])/((Algo.g[u,k] + sum(Algo.q[u,r,s,k] for r in Ntw.R for s in Ntw.S))/2) for k in Ntw.K for u in Ntw.Scn.U)
        maxsd_perc = max( abs(SD[u, k])/((Algo.c[u,k] + sum(Algo.q[u,r,s,k] for r in Ntw.R for s in Ntw.S))/2)  for k in Ntw.K for u in Ntw.Scn.U)
        EM_perc[iter] = maxee_perc
        SM_perc[iter] = maxsd_perc
        ...
        if iter > 5:
            ...
            #zg change to maxee_perc and maxsd_perc on 20220329
            if maxee_perc <= ES_tol and maxsd_perc <= SD_tol:
                ...
                if EM_perc[iter-1] <= 1*ES_tol and EM_perc[iter-2] <= 2*ES_tol and EM_perc[iter-3] <= 3*ES_tol and SM_perc[iter-1] <= 1*SD_tol and SM_perc[iter-2] <= 2*SD_tol and SM_perc[iter-3] <= 3*SD_tol:
                    if iter > Pre_Iter or (not congestion):
                        print ('Equilibrium found!')
                        print ('Iteration %i ES_{max} %f, SD_{max} %f'% (iter,maxee, maxsd))
                        print ('Iteration %i ES_{max}_perc %f, SD_{max}_perc %f'% (iter,maxee_perc, maxsd_perc))
                        break
